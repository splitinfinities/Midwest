...
export class LongShadow {
  ...
  @Prop({ reflect: true }) darkShade: number = 9;
  ...
  get dark() {
    return !!this.element.closest('.dark-mode, .light-mode').classList.contains('dark-mode');
  }
  ...
  getShadowCSS() {
    ...
    let css = Array(this.length)
      .fill('')
      .map((_item, index) => {
        const y = index * (horizontal ? -1 : 1);
        const x = index * (vertical ? -1 : 1);

        return `${y}px ${x}px var(--${this.complement ? 'complement' : 'theme'}-${this.dark ? this.darkShade : this.shade})`;
      });
