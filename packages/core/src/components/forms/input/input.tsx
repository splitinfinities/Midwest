...
export class Input {
  ...
  @Prop({ reflect: true }) descriptionTransform: "capitalize" | "uppercase" | "lowercase" | "none" | "full-width" | "full-size-kana" = "uppercase";
    ...
    return <Host class={`${this.status ? (this.valid ? "valid" : "invalid") : ""}`}>
      <div class="wrapper">
        <label>
          ...
          {this.description && <midwest-label size="small" textTransformation={this.descriptionTransform} underneath>{this.description}</midwest-label>}
        </label>
      </div>
    </Host>
  }
