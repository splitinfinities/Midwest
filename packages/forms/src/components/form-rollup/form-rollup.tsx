...
export class FormRollup {
  ...
  async update() {
    this.items.forEach((item) => {
      item.removeEventListener('item:changed', this.boundUpdate)
      item.addEventListener('item:changed', this.boundUpdate)
    });
  ...
  renderEmpty() {
    return <midwest-label class="empty cursor-pointer">
  ...
  renderTags() {
    return <div class="tags">
      {this.checked.map((checked) => <midwest-tag pill color="gray">
        <midwest-avatar
          slot="avatar"
          shape="circle"
          size="small"
  ...
  renderLabels() {
    return <Fragment>
      ...
      {!this.show && this.checked.length >= 1 && <midwest-label class="multiple cursor-pointer">
      ...
      {this.show && this.checked.length === (this.count - 1) && <midwest-label class="multiple cursor-pointer">
        {this.showCheckedContent()} and {this.showCheckedContent(1)}
      </midwest-label>}

      {this.show && this.checked.length === this.count && <midwest-label class="multiple cursor-pointer">
